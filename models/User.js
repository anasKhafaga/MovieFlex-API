  ...
  save(cb) {
    dbCon('users', async (db) => {
      try {
        ...
        const theUser = await db.insertOne(this.userData);
        const id = theUser.insertedId;
        cb(null, id);
      } catch (err) {
        cb(err);
      }
    ...
    return new Promise((resolve, reject) => { 
      ...
      dbCon('users', async (db) => { 
        try {
          ...
          // find user
          const user = await db.findOne({ '$or': [{ username: userData['username'] }, { email: userData['username'] }] }, {projection: {username: 1, password: 1, watchlist: 1}});
